function  [dmodel, perf] = dacefit_UK(S, Y, type,regr, corr, theta0, lob, upb)
...
% Check design points
[n, m] = size(S);  % number of design sites and their dimension
sY = size(Y);
...
S = (S - repmat(mS,n,1)) ./ repmat(sS,n,1);
Y = (Y - repmat(mY,n,1)) ./ repmat(sY,n,1);

% Calculate distances D between points
mzmax = n*(n-1) / 2;        % number of non-zero distances
ij = zeros(mzmax, 2);       % initialize matrix with indices
D = zeros(mzmax, m);        % initialize matrix with distances
ll = 0;
for k = 1 : n-1
  ll = ll(end) + (1 : n-k);
  ij(ll,:) = [repmat(k, n-k, 1) (k+1 : n)']; % indices for sparse matrix
  D(ll,:) = repmat(S(k,:), n-k, 1) - S(k+1:n,:); % differences between points
end
if  min(sum(abs(D),2) ) == 0
  error('Multiple design sites are not allowed'), end

% Regression matrix
F = feval(regr, type, S);  [mF,p] = size(F);
if  mF ~= n, error('number of rows in  F  and  S  do not match'), end
if  p > mF,  error('least squares problem is underdetermined'), end

% parameters for objective function
par = struct('corr',corr, 'regr',regr, 'type', type,'y',Y, 'F',F, ...
  'D', D, 'ij',ij, 'scS',sS);
...
% Return values
dmodel = struct('regr',regr, 'corr',corr,'type',type, 'theta',theta.', ...
...
m = size(par.F,1);
% Set up  R
r = feval(par.corr,par.type,theta, par.D);
idx = find(r > 0);   o = (1 : m)';   
mu = (10+m)*eps;
