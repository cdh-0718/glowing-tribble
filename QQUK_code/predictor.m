...
% Last update August 26, 2002
  ...
  if  mx == 1  % one site only
    ...
    if  nargout > 1                 % gradient/Jacobian wanted
      [f, df] = feval(dmodel.regr, dmodel.type,x);
      [r, dr] = feval(dmodel.corr, dmodel.type,dmodel.theta, dx);
%       [r dr] = corrgauss(dmodel.type , dmodel.theta, dx);
      % Scaled Jacobian
      ...
      if  nargout > 2  % MSE wanted
        ...
        f = feval(dmodel.regr, dmodel.type,x);
        r = feval(dmodel.corr, dmodel.type ,dmodel.theta.', dx);
    end
    ...
    f = feval(dmodel.regr, dmodel.type,x);
    r = feval(dmodel.corr, dmodel.type , dmodel.theta.', dx); %%这里将dmodel.theta改为转置.'
    r = reshape(r, m, mx);
